(function(e, c) {
	...
	j.fn = {
		...
		selector: function(o, r) {
			...
			if (j.isString(o)) {
				...
				selectFilter = function(u, z, y) {
					...
					case "odd":
						j.each(u,
						function(A) { (A % 2 === 1) && result.push(this)
						});
						break;
					case "even":
						j.each(u,
						function(A) { (A % 2 === 0) && result.push(this)
						});
						break;
					case "first":
						result.push(u[0]);
				...
				function(w, u) {
					...
					x = x || r;
					j.each(j.trim(u.replace(/\s{2,}/g, " ")).split(" "),
					function(v, y) {
		...
		getByClass: function(o, r, p) {
			...
			j.each(o,
			function() {
				j.each(this.getElementsByTagName(p),
				function() {
		...
		animation: function(t, z, r, x, v) {
			...
			clearTimeout(u.timer); (function() {
				var C = "";
				if (y < r) {
					y++;
					for (var A in z) {
						var B = n.test(z[A]);
						_isopacity = A === "opacity",
						_b = _isopacity ? parseFloat(q[A] * 100) / 100 : B ? q[A] : parseInt(q[A], 10),
						_c = B ? s.color10(z[A]) : _isopacity ? z[A] : parseInt(z[A], 10),
						_v = v(y, _b, (_c - _b), r),
						_v = _isopacity ? Math.ceil(_v * 100) / 100 : B ? w(v, y, _b, _c, r) : Math.ceil(_v) + "px";
						if (o) {
							if (_isopacity) {
								p.opacity = _v;
	...
	j.browser = function() {
		...
		n = "";
		j.each(["msie", "firefox", "chrome", "opera", "safari", "camino", "gecko"],
		function() {
	...
	j.each = function(q, s) {
		...
		if (r === "array" || r === "Aui" || r === "dom") {
			var n = q.length; ! n && o && s.call(q, 0, q);
			for (p = 0; p < n; p += 1) {
